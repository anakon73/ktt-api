...
test('MeetingServiceController updates Service and Meeting with shared date', function () {
  $payload = [
    'date' => '2025-02-20',
  ...
  $firstId = Meeting::min('id');

  $response = patchJson("/api/meeting-service/{$firstId}", $payload);
  ...
  expect(Meeting::where('leading', 'Alice Smith2')->where('date', '2025-02-20')->exists())->toBeTrue();
  expect(Service::where('scene', 'Main Hall2')->where('date', '2025-02-20')->exists())->toBeTrue();
});
